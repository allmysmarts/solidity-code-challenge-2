...
describe("Contract EthPool", () => {
  ...
  describe("User", () => {
    it("should not be able to deposit multiple times without withdraw", async () => {
      ...
      await expect(
        ethPool
          .connect(userA)
          .deposit({ value: ethers.utils.parseEther("100") })
      ).to.be.revertedWith("Not allowed multiple deposit");
    ...
    it("should get valid rewards", async () => {
      ...
      await ethPool
        .connect(admin)
        .addNewReward({ value: ethers.utils.parseEther("200") });
      ...
      await ethPool
        .connect(admin)
        .addNewReward({ value: ethers.utils.parseEther("100") });
      ...
      rewardsAAmount = ethers.utils.formatEther(
        currentABalance.sub(prevABalance).toString()
      );
      rewardsBAmount = ethers.utils.formatEther(
        currentBBalance.sub(prevBBalance).toString()
      );

      // ignore gas fee
      expect(Math.ceil(Number(rewardsAAmount))).to.equal(150);
      expect(Math.ceil(Number(rewardsBAmount))).to.equal(150);
    });
  });
});
